...
class AppsInfoController extends BaseController {
    /**
     * 当前模块参数
     */
	...
	$page = I('request.page',0,'intval');
        ...
        if(!empty($positionId) && empty($specialId)){
            ...
            $where['_string'] = 'find_in_set('.$specialId.',special) and find_in_set('.$positionId.',position) ';
        }
        if(!empty($status)){
            switch ($status) {
                case '1':
