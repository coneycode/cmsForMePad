...
class ContentArticleModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('score',array(0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5), '应用评分输入有误',3,'in'),
    );
     ...
     * @return array 列表
     */
    public function loadList($where = array(), $limit = 0, $order = 'A.downloads desc,A.update_time desc,A.content_id desc'){
        // $where['C.app'] = 'Article';
        $pageList = $this->table("__CONTENT__ as A")
                    ...
                    ->field('A.*,B.*,C.name as class_name,C.image as class_image')
                    ->where($where)
                    ->order($order)
                    ->select();
        //处理数据类型
