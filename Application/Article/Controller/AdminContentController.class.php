...
class AdminContentController extends AdminController {
    ...
    public function index(){
        ...
        $specialId = I('request.special_id',0,'intval');
        if(!empty($keyword)){
        ...
        if(!empty($positionId) && empty($specialId)){
            $where['_string'] = 'find_in_set('.$positionId.',position) ';
        }else if(empty($positionId) && !empty($specialId)){
            $where['_string'] = 'find_in_set('.$specialId.',special) ';
	}else if(!empty($specialId) && !empty($positionId)){
            $where['_string'] = 'find_in_set('.$positionId.',position) and find_in_set('.$specialId.',special) ';
        }
        if(!empty($status)){
            switch ($status) {
                case '1':
    ...
    //                D('DuxCms/Special')->editData($data);
                    break;
				case 5:
                    //删除
